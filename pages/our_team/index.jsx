    ...
    <Container fluid className={ styles.ourTeam_container_main }>
      ...
      <div className={ styles.ourTeam_container } >
        {data.map(( { NAME, description, members }, idx ) => (
          <>
            <div className={[  styles.ourTeam_advisoryBoard ].join(' ')} key={ NAME } >
             <SplitPhrase name={ NAME } />
            </div>

            <div className={[  styles.card_container__outer].join(' ')}  >

            {members.map(({image, name, title, description, link}, i) =>  (
               <div className={ styles.card_container__inner_main } >
                  <Card  key={i} className={['', styles.card_container__inner ].join(' ')}  >

                    <ConditionalLinks //adds an optional <Link href={...}> tag around cards with a link
                          condition={link} 
                          wrapper={ children => <Link href={link} key={name} className={ styles.conditionalLinks } ><a>{children}</a></Link> } >
                        <img src={image || '/person-placeholder-image.png'} alt={image} 
                              className={[  !image ? styles.card_placeholder_image : styles.card_image , 
                                            !name && styles.Card_hidden
                                    ].join(' ')} />

                      <Card.Title className={[ styles.card_title,  ].join(' ')} >
                        {name}
                      </Card.Title>
                      ...
                      <>
                        ...
                        <div className={[ NAME == 'Advisory Board' && styles.card_container__inner__colored__text__d].join(' ')}>
                          {description}
                        </div>
                      </>
                      :
                      <>
                        <div className={[  styles.card_container__inner__colored__text].join(' ')} >
                          {title}
                        </div>
                      </>
                      }
